...
### 📊 Cron任务错误分类与修复模式 (2026-02-20)
**关键发现**: Cron任务失败可分为三类，每类需要不同修复策略:

**错误类型分类**:
| 错误类型 | 症状 | 修复策略 | 示例 |
|----------|------|----------|------|
| **Model权限错误** | `model not allowed` | 更新任务配置中的model字段 | gateway-health-monitor |
| **超时错误** | `execution timed out` | 增加timeoutSeconds，简化任务范围 | 商家数据-实时抓取 |
| **交付错误** | `delivery failed` | 检查channel/to配置，网络连通性 | workspace-backup |

**修复优先级**:
1. **P0 - 立即修复**: Model权限错误 (导致任务完全失败)
2. **P1 - 24小时内**: 超时错误 (影响任务完成但不致命)
3. **P2 - 本周内**: 交付错误 (任务完成但通知失败)

**预防性配置最佳实践**:
- **避免硬编码模型**: 使用 `"model": "default"` 或省略model字段
- **超时配置**: 简单检查60s，数据抓取300s，复杂分析600s+
- **交付模式选择**: `none`用于高频后台任务，`announce`仅用于可执行结果
- **consecutiveErrors监控**: 连续错误>5次时触发人工干预

### 🏪 商家数据抓取系统成功模式 (2026-02-20)
**关键发现**: 商家数据抓取任务成功运行，提取可复制的模式:

**成功配置要素**:
1. **高频调度**: `*/5 * * * *` 每5分钟抓取，确保数据实时性
2. **充足超时**: `timeoutSeconds: 300` (5分钟)，匹配数据抓取复杂度
3. **明确数据路径**: 绝对路径指定数据保存位置，避免歧义
4. **自动化交付**: `mode: announce` 确保每次抓取结果通知到位

**数据异常检测模式** (2026-02-20 05:45):
- **退款警报**: ¥116.60成交被全额退回 → 触发调查建议
- **评分差异**: 美团57.5分 vs 抖音135分 → 标记平台差异
- **违规状态**: 抖音"违规生效中" → 建议立即查看详情
...
1. **双重检查**: CPU + 内存同时监控
2. **趋势分析**: 不仅看点，还要看变化趋势
3. **自动恢复**: 高CPU自动恢复，无需人工干预
4. **预警阈值**: CPU>150%持续30分钟 → 建议重启
...
### 🧠 自我改进分析 - 2026-02-21 00:00

**近期工作模式分析**:

**成功模式识别**:
1. **高频数据抓取系统**: 商家数据抓取任务 (`*/5 * * * *`) 成功运行模式
   - 配置要素: 高频调度 + 充足超时 (300s) + 明确数据路径 + 自动化交付
   - 数据异常检测: 退款警报、评分差异、违规状态自动标记
   - 运营建议生成: 数据汇总 → 异常标记 → 行动建议 → 文件归档

2. **密室逃脱运营系统**: 渐进式部署策略成功
   - 从精简版开始，逐步完善数据抓取和自动化
   - 异常预警机制: P0级别数据缺失警告确保问题及时被发现
   - 多任务协调: 每日运营分析 + 竞品监控 + 内容生成 + 周度报告

3. **网关健康监控优化**: 双重检查机制有效
   - CPU + 内存同时监控，趋势分析优于单点检查
   - 自动恢复机制: 高CPU自动恢复，无需人工干预
   - 预警阈值: CPU>150%持续30分钟 → 建议重启

**问题模式识别**:
1. **模型配置错误**: 2个任务因 `model not allowed: deepseek/deepseek-chat` 失败
   - 根本原因: 任务配置使用了已不允许的模型
   - 解决方案: 更新为 `moonshot/kimi-k2.5` 或其他允许模型

2. **超时问题**: 商家数据抓取任务超时 (60s不足)
   - 根本原因: 数据抓取需要更长时间
   - 解决方案: 增加timeoutSeconds到300s

3. **API配额问题**: 多个任务因API余额不足失败
   - 根本原因: DeepSeek API余额耗尽，Moonshot在冷却期
   - 解决方案: 需要监控API余额，设置备用provider
...
| **Model权限错误** | `model not allowed` | 更新任务配置中的model字段 | gateway-health-monitor |
| **超时错误** | `execution timed out` | 增加timeoutSeconds，简化任务范围 | 商家数据-实时抓取 |
| **API配额错误** | `API key has run out of credits` | 切换provider，监控余额 | L-150-48H-Outreach-Monitor |
| **交付错误** | `delivery failed` | 检查channel/to配置，网络连通性 | workspace-backup |
...
- **交付模式选择**: `none`用于高频后台任务，`announce`仅用于可执行结果
- **consecutiveErrors监控**: 连续错误>5次时触发人工干预
- **多provider备用**: 配置多个API provider，避免单点故障

**当前系统状态**:
- **活跃任务**: 10个 (商家数据抓取、密室逃脱监控、网关健康监控等)
- **禁用任务**: 11个 (L-150相关任务，用户指令停止)
- **错误任务**: 2个 (需要模型配置修复)
- **总体健康度**: 80% (16/20任务正常运行)

**推荐新技能安装**:
1. **cron-task-manager**: 紧急需要，用于批量修复模型配置问题
2. **api-balance-monitor**: 监控各provider API余额，提前预警
3. **task-optimizer**: 自动分析任务执行时间，优化超时配置

**工作流优化建议**:
1. **定期模型配置检查**: 每月检查所有cron任务的model配置
2. **API余额监控**: 设置每周API余额检查
3. **任务执行时间分析**: 记录任务执行时间，优化超时配置
4. **错误自动修复**: 对于常见错误类型，开发自动修复脚本

**关键教训**:
1. **模型配置集中管理**: 避免硬编码，使用环境变量或默认配置
2. **超时配置匹配复杂度**: 简单任务60s，数据抓取300s，复杂分析600s+
3. **多provider策略**: 至少配置2个备用API provider
4. **错误分类处理**: 不同错误类型需要不同修复策略
5. **预防性监控**: 在问题发生前检测和预防
