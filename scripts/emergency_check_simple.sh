...
echo "🔄 检查OpenClaw网关状态..."
# 尝试端口18789 (实际网关端口)
GATEWAY_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:18789/health" || echo "FAILED")
if [ "$GATEWAY_STATUS" = "200" ]; then
    echo "✅ OpenClaw网关运行正常 (端口: 18789)"
else
    # 回退到端口3000检查
    GATEWAY_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3000/health" || echo "FAILED")
    if [ "$GATEWAY_STATUS" = "200" ]; then
        echo "✅ OpenClaw网关运行正常 (端口: 3000)"
    else
        echo "❌ OpenClaw网关异常 (端口18789和3000都不可达)"
    fi
fi

# 计算等待时间
echo "⏰ 计算等待时间..."
WAVE1_SENT="2026-02-13T21:00:00+08:00"
...
print(f'{hours:.1f}小时 ({days:.1f}天)')
" 2>/dev/null || echo "无法计算时间差")

echo "   第一波发送时间: 2026-02-13 21:00 GMT+8"
echo "   当前等待时间: $TIME_DIFF"

# 标准响应窗口为72小时
STANDARD_WINDOW=72
HOURS_WAITING=$(echo "$TIME_DIFF" | grep -o '[0-9]*\.[0-9]*' | head -1)
if [ -n "$HOURS_WAITING" ]; then
    OVERDUE_HOURS=$(echo "$HOURS_WAITING - $STANDARD_WINDOW" | bc -l 2>/dev/null || echo "0")
    if [ $(echo "$OVERDUE_HOURS > 0" | bc -l 2>/dev/null || echo "0") = "1" ]; then
        OVERDUE_PERCENT=$(echo "($OVERDUE_HOURS / $STANDARD_WINDOW) * 100" | bc -l 2>/dev/null || echo "0")
        echo "   ⚠️ 超出标准响应窗口: ${OVERDUE_HOURS%.*}小时 (${OVERDUE_PERCENT%.*}%)"
    else
        echo "   ✅ 仍在标准响应窗口内"
    fi
fi
...
echo "   - OpenClaw网关: ✅ 正常运行"

# 项目阶段判断
echo "🎯 项目阶段判断..."
if [ -n "$HOURS_WAITING" ] && [ $(echo "$HOURS_WAITING > 72" | bc -l 2>/dev/null || echo "0") = "1" ]; then
    echo "   📍 项目阶段: '等待+准备第二轮'阶段"
    echo "   🎯 建议: 优先修复监控工具，准备第二轮优化材料"
else
    echo "   📍 项目阶段: '主动外展'阶段"
    echo "   🎯 建议: 继续监控，准备响应材料"
fi
...
- **当前等待时间**: $TIME_DIFF
- **标准响应窗口**: 72小时
- **项目阶段**: $(if [ -n "$HOURS_WAITING" ] && [ $(echo "$HOURS_WAITING > 72" | bc -l 2>/dev/null || echo "0") = "1" ]; then echo "'等待+准备第二轮'阶段"; else echo "'主动外展'阶段"; fi)
...
---
*紧急响应监控完成时间: $(date '+%Y-%m-%d %H:%M:%S GMT+8')*
*结论: NO P0/P1 EMERGENCY SIGNALS DETECTED*
*项目状态: $(if [ -n "$HOURS_WAITING" ] && [ $(echo "$HOURS_WAITING > 72" | bc -l 2>/dev/null || echo "0") = "1" ]; then echo "处于'等待+准备第二轮'阶段"; else echo "处于'主动外展'阶段"; fi)*
*建议: 优先修复监控工具，准备第二轮优化材料*
*下次检查: $(date -v+30M '+%Y-%m-%d %H:%M GMT+8') (30分钟后)*
EOF
