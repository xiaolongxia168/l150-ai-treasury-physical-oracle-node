...
# 计算等待时间（简单方法）
echo "⏰ 计算等待时间..."
WAVE1_TIMESTAMP=$(date -j -f "%Y-%m-%d %H:%M:%S" "2026-02-13 21:00:00" "+%s" 2>/dev/null || echo "0")
CURRENT_TIMESTAMP=$(date "+%s")
if [ "$WAVE1_TIMESTAMP" != "0" ]; then
    DIFF_SECONDS=$((CURRENT_TIMESTAMP - WAVE1_TIMESTAMP))
    HOURS_WAITING=$(echo "scale=1; $DIFF_SECONDS / 3600" | bc)
    DAYS_WAITING=$(echo "scale=1; $HOURS_WAITING / 24" | bc)
    echo "   第一波发送时间: 2026-02-13 21:00 GMT+8"
    echo "   当前等待时间: ${HOURS_WAITING}小时 (${DAYS_WAITING}天)"
    
    # 标准响应窗口为72小时
    STANDARD_WINDOW=72
    if [ $(echo "$HOURS_WAITING > $STANDARD_WINDOW" | bc) -eq 1 ]; then
        OVERDUE_HOURS=$(echo "$HOURS_WAITING - $STANDARD_WINDOW" | bc)
        OVERDUE_PERCENT=$(echo "scale=1; ($OVERDUE_HOURS / $STANDARD_WINDOW) * 100" | bc)
...
# 项目阶段判断
echo "🎯 项目阶段判断..."
if [ "$HOURS_WAITING" != "0" ] && [ $(echo "$HOURS_WAITING > 72" | bc) -eq 1 ]; then
...
### ⏰ 时间线更新
- **当前等待时间**: ${HOURS_WAITING}小时 (${DAYS_WAITING}天)
- **标准响应窗口**: 72小时
$(if [ "$HOURS_WAITING" != "0" ] && [ $(echo "$HOURS_WAITING > 72" | bc) -eq 1 ]; then 
    echo "- **超出标准窗口**: ${OVERDUE_HOURS}小时 (${OVERDUE_PERCENT}%)"
    echo "- **项目阶段**: '等待+准备第二轮'阶段"
else
    echo "- **项目阶段**: '主动外展'阶段"
fi)
...
$(if [ "$HOURS_WAITING" != "0" ] && [ $(echo "$HOURS_WAITING > 72" | bc) -eq 1 ]; then 
    echo "*项目状态: 处于'等待+准备第二轮'阶段*"
else
    echo "*项目状态: 处于'主动外展'阶段*"
fi)
