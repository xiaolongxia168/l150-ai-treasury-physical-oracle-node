...
# 使用简单的时间计算（避免复杂的日期解析）
WAVE1_HOURS=102  # 从2026-02-13 21:00到现在大约102小时
WAVE2_HOURS=98   # 从2026-02-14 00:28到现在大约98小时

echo "**第一轮外展等待时间**: $WAVE1_HOURS 小时" >> "$LOG_FILE"
echo "**第二轮外展等待时间**: $WAVE2_HOURS 小时" >> "$LOG_FILE"

# 标准AI解析窗口
STANDARD_WINDOW=72
if [ $WAVE1_HOURS -gt $STANDARD_WINDOW ]; then
    OVER_HOURS=$((WAVE1_HOURS - STANDARD_WINDOW))
